...
# # 🔥 MINIMAL CHANGE: ONE helper to ensure all steps use cleaned data
...
#     # numeric → categorical if low unique
...
#     # 🔥 Persist cleaned dataset for entire workflow
...
#         "is_numeric": y.dtype.kind in "iuf"
#     }
...
#     selector = SelectKBest(scorer, k=min(k, len(numeric)))
#     selector.fit(X[numeric], y)

#     scores = selector.scores_
#     pairs = sorted(zip(numeric, scores), key=lambda x: x[1], reverse=True)
...
#         "feature_scores": [{"feature": f, "score": float(s)} for f, s in pairs],
...
#     # 🔥 MINIMAL ADDITION: NEW MODEL OPTIONS
...
#     pipeline = session.get("pipeline") or joblib.load("trained_model.pkl")
...
#     print(session)
#     print(session.get('target_column'))
#     target_column = session.get("target_column") or df.columns[-1]

#     if target_column not in df.columns:
#         raise ValueError("Target column not found in uploaded file.")
...
#     # Convert numpy prediction array
#     preds = [p.item() if isinstance(p, (np.generic, np.ndarray)) else p for p in preds]
...
# def download_model():
#     return FileResponse("trained_model.pkl", filename="model.pkl")
